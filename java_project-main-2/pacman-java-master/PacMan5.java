...
public class PacMan5 extends JPanel implements ActionListener, KeyListener {
    class Block {
        ...
        int velocityX = 0;
        int velocityY = 0;
        char ghostType; // Hayalet tipini tutacak değişken ekledik
    ...
    PacMan5() {
        ...
        gameLoop = new Timer(33, this); // 1000ms/30fps ≈ 33ms
        gameLoop.start();
    ...
    private Block createGhost(char tile, int x, int y) {
        Block ghost = new Block(getGhostImage(tile), x, y, tileSize, tileSize);
    ...
    public void move() {
        ...
        // Güç yemi kontrolü
        Iterator<Block> powerUpIterator = powerUps.iterator();
        while (powerUpIterator.hasNext()) {
            Block powerUp = powerUpIterator.next();
        ...
        for (Block powerUp : powerUps) {
            if (collision(pacman, powerUp)) {
                cherryEaten = powerUp;
                score += CHERRY_SCORE; // Kiraz yendiğinde 300 puan
            }
        }
        if (cherryEaten != null) {
